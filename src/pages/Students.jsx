...
export default function Students() {
  ...
  const columns = [
    ...
    // { key: 'date_of_birth', label: 'Date of Birth', render: (_, row) => row.user?.date_of_birth || '—' },
  ]
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      role_id: crud.formData.role_id ?? user.role_id ?? 4,
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Students" description="Manage student records" onAdd={() => crud.openAdd({ role_id: 4, user: {} })} addLabel="Add Student" />
      ...
      {!crud.loading && crud.paginated.length === 0 ? <EmptyState title="No students found"
        action={() => crud.openAdd({ role_id: 4, user: {} })} actionLabel="Add Student" /> : (
        <>
      ...
      <Modal open={crud.modalOpen} onClose={crud.closeModals} title={crud.selected ? 'Edit Student' : 'Add Student'} size="xl" footer={<><Button variant="secondary" onClick={crud.closeModals}>Cancel</Button><Button onClick={save} disabled={crud.loading}>{crud.selected ? 'Update' : 'Create'}</Button></>}>
        <div className="space-y-6">
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 space-y-1.5">
              <h3 className="text-sm font-semibold text-surface-900 dark:text-white">
                {crud.formData.user?.name || 'Student Profile'}
              </h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField label="Full Name" required><Input value={crud.formData.user?.name || ''} onChange={e => updateUserField('name', e.target.value)} placeholder="Student name" /></FormField>
                ...
                <div className="space-y-3">
                  <div className="flex justify-center py-2">
                    <img 
                      ...
                      alt={crud.selected.user?.name || 'User'} 
      ...
      <ConfirmDialog open={crud.deleteModal} onClose={crud.closeModals} onConfirm={del} title="Delete Student" message={`Delete "${crud.selected?.user?.name}"?`} />
    </div>
  )
}